...
int main(int argc, char* argv[])
{
	...
	*/
	vector<future<TreeNode*>> threads;
	vector<TreeNode*> threadtree(seglist.size());
	while(treelist.size() > 1)
	{
		for(int i=0; i<treelist.size()-1;i+=2)
		{
			future<TreeNode*> test_future;
			test_future = async(build_tree,treelist[i],treelist[i+1]);
			threads.push_back(test_future);	
		...
		{
			TreeNode* temporary = threads[j].get();
			cout<<temporary->sum;
			treelist.push_back(temporary);
		}
